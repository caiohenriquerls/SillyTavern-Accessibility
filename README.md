...
- **select2 dropdowns** — SillyTavern hides the real `<select>` behind the select2 widget and marks it `aria-hidden` + `tabindex="-1"`, so a screen reader cannot reach it. This re‑exposes the native `<select>` (a control NVDA operates well) and hides the visual widget from the accessibility tree, giving a single, operable tab stop. This is what finally lets a screen‑reader user **activate a lorebook** in *Active World(s) for all chats* (and use the model pickers, provider lists, etc.). The mouse keeps using the visual widget unchanged. For **multiselects**, each selected item's "x" is also exposed as a real **"Deactivate \<name\>"** button, so an active lorebook (or any selected item) can be turned off with a single Enter — no modifier keys.
